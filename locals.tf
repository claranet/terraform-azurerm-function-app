locals {
  ...
  function_name_prefix = var.function_app_name_prefix != "" ? replace(var.function_app_name_prefix, "/[a-z0-9]$/", "$0-") : local.function_name_prefix
  ...
  storage_account_connection_string = var.create_storage_account_resource == "true" ? join(
    "",
    azurerm_storage_account.storage.*.primary_connection_string,
  ) : var.storage_account_connection_string

  app_insights_instrumentation_key = var.create_application_insights_resource == "true" ? join(
    "",
