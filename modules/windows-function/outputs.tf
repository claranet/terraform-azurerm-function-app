...
output "connection_string" {
  ...
  value       = azurerm_windows_function_app.main.connection_string
  sensitive   = true
}

output "identity_principal_id" {
  description = " Function App system identity principal ID."
  value       = try(azurerm_windows_function_app.main.identity[0].principal_id, null)
}

output "resource" {
  description = "Function App resource object."
  value       = azurerm_windows_function_app.main
}
...
output "slot_name" {
  ...
  value       = try(azurerm_windows_function_app_slot.staging[0].name, null)
}

output "slot_default_hostname" {
  description = "Default hostname of the Function App slot."
  value       = try(azurerm_windows_function_app_slot.staging[0].default_hostname, null)
}

output "slot_identity" {
  description = "Identity block output of the Function App slot."
  value       = try(azurerm_windows_function_app_slot.staging[0].identity[0], null)
}

output "resource_slot" {
  description = "Function App staging slot resource object."
  value       = one(azurerm_windows_function_app_slot.staging[*])
}
