...
| storage\_account\_shared\_access\_key\_enabled | Whether to the shared access keys are enabled for the Storage Account. `storage_use_azuread` flag in the azurerm must be enabled if access key is disabled. | `bool` | `false` | no |
